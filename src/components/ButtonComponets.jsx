...
const ButtonComponents = ({ darkmode }) => {
  return (
...
const StyledWrapper = styled.div`
  .button {
    cursor: pointer;
    border: solid 4px ${({ darkmode }) => (darkmode ? "#fff" : "#161616")};
    border-top: none;
    border-radius: 20px;
    position: relative;
  ...
  .inner {
    ...
    font-weight: 600;
    letter-spacing: 1px;
    border-bottom: solid 3px ${({ darkmode }) => (darkmode ? "#fff" : "#374e72")};
    border-radius: 16px;
    background: ${({ darkmode }) =>
      darkmode
        ? "linear-gradient(180deg, #333, #000)"
        : "linear-gradient(180deg, #5771a5, #000)"};
    color: ${({ darkmode }) => (darkmode ? "#fff" : "#fff")};
    text-shadow: 1px 1px #000, 0 0 9px ${({ darkmode }) => (darkmode ? "#fff" : "#fff")};
  }
  ...
  .svgs > * {
    filter: drop-shadow(0 0 6px ${({ darkmode }) => (darkmode ? "#fff" : "#000")})
      drop-shadow(1px 1px 0px ${({ darkmode }) => (darkmode ? "#000" : "#000")});
  }
